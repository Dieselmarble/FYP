...
%   $Revision: 2.10.2.3 $  $Date: 2004/07/28 04:38:35 $
...
if flag
    ...
    y = nanmedian2(abs(x - repmat(nanmedian2(x,dim), tile)),dim);
else
    % Compute the mean of the absolute deviations from the mean.
    y = nanmean2(abs(x - repmat(nanmean2(x,dim), tile)),dim);
end
