function [Wefica, ISRef, Wsymm, ISRsymm, status, icasig]=efica(X,m,ini, SaddleTest)
...
ISRef=Werr/N;
Wefica=Wefica'*CC;
% [vars,keys]	= sort(sum(Wefica.*Wefica)) ;
% Wefica		= Wefica(keys,:);
% Wefica		= Wefica(m:-1:1,:) ; % Is this smart ?
Wsymm=Wsymm'*CC;
icasig=Wefica*X + (Wefica*Xmean)*ones(1,N);
% 
% for i = 1:10
%     figure;
%     imshow(reshape(icasig(i,:),256,256),[]);
% end
