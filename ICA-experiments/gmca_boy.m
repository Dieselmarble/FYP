...
meanValue = 0;
var_gauss = 0.3;
imgto = 0.4*double(imread('texture4.tif'));
imgco = 0.6*double(imread('boy.tif'));

qmf = MakeONFilter('Battle',5);
imgt = FWT2_PO(double(imgto),1,qmf);
imgc = FWT2_PO(double(imgco),1,qmf);
...
%     [icasig,A,W] = fastica (tempy.', 'numOfIC', nComps, 'verbose','off');
    ...
    C{1} = IWT2_PO(C{1},1,qmf); % mixtures
    C{2} = IWT2_PO(C{2},1,qmf);

    R1 = corr2(normalize(imgto),normalize(C{1}));
    R2 = corr2(normalize(imgco),normalize(C{2}));
    R1 = abs(R1);
    R2 = abs(R2);
    R_all = [R_all;R2];
