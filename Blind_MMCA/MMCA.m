...
nmax= 400; % maximum number of iterations
% add noise
SNR_db = 3;
...
% ----------------- initialisation of the SB-SVD algorithm ------------
...
% Dictionary = 1:K;         %block structure with K blocks of size 1 (i.e. block structure ignored)
% d = repmat(1:B, s,1); d = d(:)';  %block structure with B blocks of size s
% M = 16; %number of samples after compression
% 
% Dictionary = BKSVD(Y,K,k,s,Dictionary,max_it);

% ----------------- end of the SB-SVD algorithm ------------
tic
for (pp=1:nmax)
    ...
    for ff = 1:NbSources
        ...
        for jj = 1:numIteration
            ...
            All_Coef{ff} = CoefMatrix; % coefficients of nb sources decomopsed in the union dictionary
...
%             %===============================================================
            ...
            % --------------------------------------------------------------- %
%             Dictionary = BKSVD(Data,K,k,s,Dictionary,max_it);
            % --------------------------------------------------------------- %
...
%                 temp = temp(:).*(abs(temp(:)) > KS*mad2(temp(:))); %soft-thresholding
%                 my_y = IWT2_PO(reshape(temp,size(im1)),1,qmf);
%                 SEst_r(ff,:) = my_y(:); %faltten it
                %---------------------------------------------------------------
...
%         
        switch(params.trans)
            case  'wavelet'
                temp = FWT2_PO(my_y,1,qmf); % wavelet transform
                temp = temp(:).*(abs(temp(:)) > KS*mad2(temp(:)));
                my_y2 = IWT2_PO(reshape(temp,imsize),1,qmf);
            case 'curv' % curvelet transform
                temp = dct2(my_y);
                temp = temp(:).*(abs(temp(:)) > KS*mad2(temp(:)));
...
R1 = corr2(im1,im_source_noisy1);
R2 = corr2(im2,im_source_noisy2);
R3 = corr2(im3,im_source_noisy3);
R4 = corr2(im4,im_source_noisy4);
...
% 
% figure(5);
% dictimg = showdict(Dictionary,[1 1]*patchsz,round(sqrt(dicsz)),round(sqrt(dicsz)),'lines','highcontrast');
